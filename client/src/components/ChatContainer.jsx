  ...
  return selectedUser ?(
    ...
    <div className='h-full overflow-scroll relative bg-white'>
      {/*-------header------*/}
        ...
        <div className='flex items-center gap-3 py-3 mx-4 border-b border-sky-200'>
          <img src={selectedUser.profilePic || assets.avatar_icon} alt="" className="w-8 rounded-full"/>
          
          <p className='flex-1 text-lg text-slate-700 flex items-center gap-2'> 
          ...
          {messages?.map((msg,index)=>(
            <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !==authUser._id && 'flex-row-reverse'}`}>
              {msg.image ?(
                ...
                border-sky-200 rounded-lg overflow-hidden mb-8'/>
              ):(
                <p className={`p-2 max-w-[200px] md:text-sm font-light
                                   rounded-lg mb-8 break-all ${msg.senderId=== authUser._id
                    ? 'bg-sky-500 text-white rounded-br-none'
                    : 'bg-sky-100 text-slate-700 border border-sky-200 rounded-bl-none'}`}>
              ...
              <div className="text-center text-xs">
                ...
                rounded-full'/> 
                <p className='text-slate-400'>{formatMessageTime (msg.createdAt)}</p>
              </div>
            </div>
          ))}
        ...
        <div className='flex-1 flex items-center bg-sky-50 border border-sky-200 px-3 rounded-full'>
            <input onChange={(e)=>setInput(e.target.value)} value={input}
            onKeyDown={(e)=>e.key==="Enter" ? handleSendMessage(e):null}
            type="text" placeholder="Send a message" 
            className="flex-1 text-sm p-3 border-none rounded-lg outline-none
           text-slate-700 placeholder-sky-400"/>
    ...
    bg-sky-50 max-md:hidden'> 
    
      <img src={assets.logo_icon} className='max-w-16' alt="" />
      <p className='text-lg font-medium text-sky-600'>Chat anytime,anywhere</p>
    </div>
  )
}
